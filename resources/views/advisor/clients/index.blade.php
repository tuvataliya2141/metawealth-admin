                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_clients_table">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-end min-w-100px">Notes From Contact</th>
                                <th class="text-end min-w-100px">Followup Date</th>
                            ...
                            @foreach($clients as $client)
                                <tr>
                                    ...
                                    <td class="text-end pe-0">
                                        <span class="fw-bold">{{ ($client->phone_number) ? $client->phone_number : '-' }}</span>
                                    </td>
                                    <td class="text-end pe-0">
                                        <span class="fw-bold">{{ $client->email }}</span>
                                    </td>
                                    <td class="text-end pe-0"> {{ ($client->notes_from_contact) ? $client->notes_from_contact : '-' }} </td>
                                    <td> {{ Carbon\Carbon::parse($client->followup_date)->format('M d, Y') }} </td>
                                    <td class="text-end pe-0" data-order="Published">
                                        @if($client->status == 'Warm')
                                            <div class="badge badge-light-warning">Warm</div>
                                        @elseif($client->status == 'Hot')
                                            <div class="badge badge-light-danger">Hot</div>
                                        @elseif($client->status == 'Cold')
                                            <div class="badge badge-light-primary">Cold</div>
                                        @endif
                                    </td>
                                    <td> 
                                        <div class="form-switch">
                                            <input class="form-check-input" type="checkbox" value="yes" id="client" name="client" data-id="{{ $client->id }}" onChange="convertIntoLeads(this)"/>
                                        </div>    
                                    </td>
                                    <td class="text-end">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                                            {{-- <div class="menu-item px-3">
                                                <a href="{{ route('advisorViewClient', $client->id) }}" class="menu-link px-3"> View </a>
                                            </div> --}}
                                            <div class="menu-item px-3">
                                                <a href="{{ route('advisorUpdateClient', $client->id) }}" class="menu-link px-3"> Edit </a>
                                            </div>
                                            <div class="menu-item px-3">
                                                <a href="{{ route('adminUnAssignAdvisor', $client->id) }}" class="menu-link px-3">
                                                    Unassign
                                                </a>
                                            </div>
                                        </div>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        url : "clients/statusUpdateClient/"+clientId,
                        type : 'get',
                        dataType : 'json',
